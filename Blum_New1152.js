...
// @grant        none
// @icon         https://blog.pintu.co.id/wp-content/uploads/2024/09/blum-crypto-1.jpg
...
try {
	...
	function clickElement(item) {
		...
		const createEvent = (type, EventClass) => new EventClass(type, {
			...
			pointerId: 1,
			isPrimary: true,
			pressure: type === 'touchstart' ? 0.5 : 0
		});
	
		setTimeout(() => {
			if (typeof item.onClick === 'function') {
				if (item.element) {
					// Sử dụng sự kiện cảm ứng thay vì sự kiện chuột
					['touchstart', 'touchend'].forEach(type => {
						item.element.dispatchEvent(createEvent(type, TouchEvent));
					});
				}
				item.onClick(item);
			}
	...
	telegramButton.href = 'https://t.me/DongLaoChannel';
